...
class UserRelationshipCheckView(APIView):
    ...
    def get(self, request, uuid, uuid_2):
        ...
        if user2 in user1.friends:
            return Response(data={'authors': [user1.id, user2.id], 'friends': True}, status=status.HTTP_200_OK)
        else:
            return Response(data={'authors': [user1.id, user2.id], 'friends': False}, status=status.HTTP_200_OK)
